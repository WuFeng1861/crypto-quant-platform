  ...
  try {
    ...
    if (response.data.success && response.data.generatedStrategy) {
      ...
      strategy.conditions.forEach((condition, index) => {
        const action = condition.action || '❌ 缺失';
        const compare = condition.comparisonType === 'constant' 
          ? `常量 ${condition.constantValue}` 
          : `指标 ${condition.comparedIndicatorIndex}`;
        console.log(`  [${index}] action=${action}: 指标${condition.indicatorIndex} ${condition.operator} ${compare}`);
      });
      
      const missingActions = strategy.conditions.filter(c => !c.action);
      if (missingActions.length > 0) {
        console.log('\n⚠️ 警告: 有条件缺少 action 字段!');
      }
    }
...
async function runAllTests() {
  ...
  await testGenerateStrategy(
    '一个能够在右侧中部行情下面的策略，至少使用三个指标，交易纪律需要偏激进，严格控制止损位',
    '测试2: 复杂策略 (测试数据验证)'
  );
