...
// 创建SMA指标的函数
async function createSMAIndicator() {
  try {
    ...
    console.log('SMA指标创建成功:');
    console.log(JSON.stringify(response.data, null, 2));
    return response.data;
  } catch (error) {
    console.error('创建SMA指标失败:', error.response ? error.response.data : error.message);
    throw error;
  }
}

// 测试SMA指标的函数
async function testSMAIndicator(indicatorId) {
  try {
    const testData = {
      priceData: [
        {"close_price": 100},
    ...
    console.log('SMA指标测试结果:');
    console.log(JSON.stringify(response.data, null, 2));
    return response.data;
  } catch (error) {
    console.error('测试SMA指标失败:', error.response ? error.response.data : error.message);
    throw error;
  }
}
...
async function main() {
  try {
    // 创建SMA指标
    const createResult = await createSMAIndicator();
    
    // 如果创建成功，测试该指标
    if (createResult && createResult.data && createResult.data.id) {
      await testSMAIndicator(createResult.data.id);
    } else {
      ...
      if (allIndicators.data && allIndicators.data.data && allIndicators.data.data.length > 0) {
        ...
        await testSMAIndicator(lastIndicator.id);
      }
    }
  } catch (error) {
